...
async function optimizeImage(filePath: string): Promise<ImageStats> {
  ...
  // Use mogrify for in-place optimization with format-specific settings
  let command: string;
  ...
  if (needsResize) {
    switch (extension) {
      case 'jpg':
      case 'jpeg':
        command = `magick mogrify -resize ${MAX_WIDTH}x${newHeight} -strip -interlace Plane -gaussian-blur 0.05 -quality 85% -define jpeg:fancy-upsampling=off -define png:compression-filter=5 -define png:compression-level=9 -define png:compression-strategy=1 -define png:exclude-chunk=all -colorspace sRGB "${filePath}"`;
        break;
      case 'png':
        command = `magick mogrify -resize ${MAX_WIDTH}x${newHeight} -strip -define png:compression-filter=5 -define png:compression-level=9 -define png:compression-strategy=1 -define png:exclude-chunk=all -colorspace sRGB "${filePath}"`;
        break;
      case 'webp':
        command = `magick mogrify -resize ${MAX_WIDTH}x${newHeight} -strip -quality 85 -define webp:method=6 -define webp:pass=10 -define webp:target-size=0 "${filePath}"`;
        break;
      default:
        command = `magick mogrify -resize ${MAX_WIDTH}x${newHeight} -strip -quality 85 "${filePath}"`;
    }
  } else {
    switch (extension) {
      case 'jpg':
      case 'jpeg':
        command = `magick mogrify -strip -interlace Plane -gaussian-blur 0.05 -quality 85% -define jpeg:fancy-upsampling=off -define png:compression-filter=5 -define png:compression-level=9 -define png:compression-strategy=1 -define png:exclude-chunk=all -colorspace sRGB "${filePath}"`;
        break;
      case 'png':
        command = `magick mogrify -strip -define png:compression-filter=5 -define png:compression-level=9 -define png:compression-strategy=1 -define png:exclude-chunk=all -colorspace sRGB "${filePath}"`;
        break;
      case 'webp':
        command = `magick mogrify -strip -quality 85 -define webp:method=6 -define webp:pass=10 -define webp:target-size=0 "${filePath}"`;
        break;
      default:
        command = `magick mogrify -strip -quality 85 "${filePath}"`;
    }
  }
...
async function main() {
  console.log('🖼️  Starting conservative image optimization...\n');
  ...
  console.log('✅ Conservative image optimization complete!');
}
