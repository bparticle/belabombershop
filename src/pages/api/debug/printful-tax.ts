...
export default async function handler(
  ...
  try {
    ...
    console.log('🔍 Full Printful response:', JSON.stringify(result, null, 2));

    res.status(200).json({
      message: 'Printful response logged to console',
      costs: result.costs,
      calculation: {
        ...
        calculatedRate: result.costs?.subtotal ? (result.costs.vat / result.costs.subtotal) * 100 : 'N/A'
      }
    });
  } catch (error) {
